...
if __name__ == "__main__":
    ...
    #Note: we should sync with every ranker and ensure local rank 0 finishing downloading the model assets from s3 successfully. 
    comm.Barrier()
    
    model = AutoModelForCausalLM.from_pretrained(model_name_or_path, use_cache=False)
    tokenizer = LlamaTokenizer.from_pretrained(model_name_or_path, model_max_length=args.model_max_length, padding_side="right", padding='max_length', truncation=True)
    ...
    training_args = TrainingArguments(
        ...
        max_steps = 200,      #just for test
        #torch_compile=True, # optimizations for pytorch 2.0
        #eval_steps  = 60,
    ...
    # Saves the model to tmp dir and upload the model to S3 by use of s5cmd
    save_model_dir = '/tmp/output/asset/'
    tokenizer.save_pretrained(save_model_dir)
    trainer.save_model(save_model_dir)
    
    #Note: we just use the global rank 0 process in training cluster to upload the trained model assets to S3 by s5cmd command.
    if global_rank == 0:
